...
| **D-dash-1** | Dashboard xếp hạng theo **số ngày học** (`count(distinct activity_date)`), không theo điểm (2026-07-22, #6) | Hiện thực hoá D15. `leaderboard_view` thêm cột `active_days`; backend sắp xếp giảm dần theo `(active_days, lessons_completed, total_questions_done)`. |
| **D-dash-2** | Thư viện biểu đồ = **Recharts**, không Tremor | CLAUDE.md ưu tiên Tremor nhưng Tremor v3 nhắm Tailwind v3 / React 18; stack thực tế là **Tailwind v4 + React 19** nên Tremor dễ vỡ. Recharts là fallback được phép, chạy sạch (`npm run build` + `oxlint` xanh). Ghi chú: Recharts v3 kéo theo Redux Toolkit + d3 → bundle >500kB (advisory Vite, chưa cần code-split cho nhóm nhỏ). |
| **D-dash-3** | Dữ liệu dashboard đi qua **backend** (`routers/dashboard.py`), leaderboard cũng đọc view phía server | Nhất quán D21: frontend không đọc thẳng Supabase. Hai endpoint `GET /api/dashboard/me` (streak/weekly/completion, riêng tư, lọc `user_id`) và `GET /api/dashboard/leaderboard` (đọc `leaderboard_view`, gắn `is_me`). |
| **D-dash-4** | Sửa lỗi **fan-out** của `leaderboard_view` (migration `0009`) | View `0004` join `lesson_progress` VÀ `daily_activity` trong một truy vấn → `sum(questions_done_count)` bị nhân theo số bài done. `0009` tổng hợp mỗi bảng bằng subquery riêng (không cross-join). Vẫn là security-definer view (chỉ lộ số liệu tổng hợp). |
| **D-dash-5** | Mốc ngày = **giờ VN (UTC+7)**, áp cả nơi ghi lẫn nơi đọc; streak reset khi qua 0h VN mà chưa học | Helper chung `app/util/dates.py` (`vn_today`/`vn_date_of`). `quiz.py` (#4) đổi bucket `daily_activity.activity_date` từ UTC sang ngày VN để streak/biểu đồ khớp dòng đã lưu quanh nửa đêm. `created_at` vẫn lưu UTC ISO — chỉ đổi cách gom ngày. |
